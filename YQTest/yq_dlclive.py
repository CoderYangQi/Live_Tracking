...
class YQDLCLive(object):
    ...
    def get_pose(self, frame=None, **kwargs):
        ...
        if drawPic:
            drawPose = self.pose
            for i in range(drawPose.shape[0]):
                color = (0, 255, 0)  # 绿色
                # frame = cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)
                if drawPose[i, 2] > self.pcutoff:
                    frame = cv2.circle(frame, (drawPose[i, 0:2]).astype(np.int32), self.radius, color, -1)
